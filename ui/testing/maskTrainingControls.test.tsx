...
assert.match(source, /maskAvailability !== 'available'/);
...
const page = readFileSync(resolve(process.cwd(), 'src/app/jobs/new/page.tsx'), 'utf8');
assert.ok(page.indexOf('validateTrainingJobForSave(jobConfig)') < page.indexOf(".post('/api/jobs'"),
  'client mask validation runs before any save request is issued');

let current!: TrainConfig;
function Harness({ availability }: { availability: 'available' | 'unavailable' | 'pending' }) {
  ...
  current = train;
  return <InvertedMaskPriorControl train={train} maskAvailability={availability} setTrain={setTrain} />;
}
async function main() {
  let renderer!: TestRenderer.ReactTestRenderer;
  await act(async () => { renderer = TestRenderer.create(<Harness availability="available" />); });
  await act(async () => { renderer.update(<Harness availability="pending" />); });
  assert.equal(current.inverted_mask_prior, true, 'pending mask discovery does not clear an enabled prior');
  await act(async () => { renderer.update(<Harness availability="available" />); });
  assert.equal(current.inverted_mask_prior, true, 'masked-to-masked discovery preserves the enabled prior');
  await act(async () => { renderer.update(<Harness availability="unavailable" />); });
