...
async function run() {
  ...
  try {
    ...
    await writeFile(join(version, 'manifest.json'), JSON.stringify({ files: [{ managed_path: 'media/a.png', media_bytes: 5, media_sha256: sha('media'), mask_path: 'masks/a.png', mask_bytes: 3, mask_sha256: sha('png'), mask_missing: false }] }));
