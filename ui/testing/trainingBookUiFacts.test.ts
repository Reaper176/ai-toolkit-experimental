...
for (const [label, sourceText, sourceName, symbol, projected] of [
  [
    'ordinary SelectInput derived override reached from base constructor fails closed',
    ordinaryClassEffectControl(
      `class BaseMutator {
         constructor() { this.mutate(); }
         mutate() {}
       }
       class IntermediateMutator extends BaseMutator {}
       class DerivedMutator extends IntermediateMutator {
         mutate() { modeOptions.push({ value: 'b', label: 'B' }); }
       }
       const MutatorAlias = DerivedMutator;`,
      'new MutatorAlias();',
  ...
  [
    'projected options derived override reached from base constructor fails closed',
  ...
  try {
    collectVisibleControlClaimsFromSource(sourceText, sourceName, symbol, false, projected);
    factualUiContractFailures.push(label);
  } catch (error) {
    assert.match(String(error), /select.*options|accepted values|recursive|provenance/i, `${label} must honor runtime virtual dispatch`);
  }
}
try {
  const inheritedDispatchControl = collectVisibleControlClaimsFromSource(ordinaryClassEffectControl(
    `class SafeBase {
       constructor() { this.touch(); }
       touch() { return 1; }
     }
     class SafeDerived extends SafeBase {
       touch() { return super.touch(); }
     }
     const SafeAlias = SafeDerived;`,
    'new SafeAlias();',
  ), 'fixture.tsx', 'Fixture');
  if (JSON.stringify(inheritedDispatchControl[0]?.value_contract.accepted_values) !== JSON.stringify([
    { kind: 'string', value: 'a' },
  ])) factualUiContractFailures.push('safe inherited override and super dispatch remain harmless');
} catch {
  factualUiContractFailures.push('safe inherited override and super dispatch remain harmless');
}
try {
  collectVisibleControlClaimsFromSource(ordinaryClassEffectControl(
    ...
    'new RecursiveDerived();',
  ), 'fixture.tsx', 'Fixture');
  factualUiContractFailures.push('runtime-dispatched recursive override fails closed');
} catch (error) {
  assert.match(String(error), /recursive|provenance/i, 'runtime-dispatched recursive override must fail closed without hanging');
}

for (const [label, sourceText, symbol, projected] of [
  [
    'ordinary SelectInput unresolved conditional class branch fails closed',
    `import { ExternalMutator } from './external';
     const modeOptions = [{ value: 'a', label: 'A' }];
     class Safe {}
     const Selected = condition ? Safe : ExternalMutator;
     new Selected();
     function Fixture({ jobConfig, setJobConfig }) {
       return <SelectInput
         label="Mode"
  ...
  [
    'projected options unresolved conditional class branch fails closed',
    projectedOptionsControl(
      `import { ExternalMutator } from './external';
       class Safe {}
       const Selected = condition ? Safe : ExternalMutator;
  ...
  try {
    ...
    assert.match(String(error), /class|constructor|select.*options|accepted values|provenance/i, `${label} must not discard an unresolved branch`);
  }
}
try {
  const finiteSafeClassBranches = collectVisibleControlClaimsFromSource(ordinaryClassEffectControl(
    `class SafeA {}
     class SafeB {}
     const Selected = condition ? SafeA : SafeB;`,
    'new Selected();',
  ), 'fixture.tsx', 'Fixture');
  if (JSON.stringify(finiteSafeClassBranches[0]?.value_contract.accepted_values) !== JSON.stringify([
    { kind: 'string', value: 'a' },
  ])) factualUiContractFailures.push('finite all-local safe class branches remain analyzable');
} catch {
  factualUiContractFailures.push('finite all-local safe class branches remain analyzable');
}
...
const quantizationFallbackSource = (
  primaryPath: string,
  ...
  function Fixture({ jobConfig, setJobConfig }) {
    return <SelectInput
      ...
      onChange={value => {
        if (value === '') {
          setJobConfig(false, '${secondaryPath}');
          value = ${fallback};
        } else {
          setJobConfig(true, '${secondaryPath}');
        }
        setJobConfig(value, '${primaryPath}');
      }}
      options={[{ value: '', label: 'Disabled' }, { value: 'qfloat8', label: 'qfloat8' }]}
    />;
  }
...
for (const [label, primaryPath, secondaryPath] of [
  [
    'Transformer arbitrary qtype fallback fails closed',
    'config.process[0].model.qtype',
    'config.process[0].model.quantize',
  ],
  [
    'Text Encoder arbitrary qtype fallback fails closed',
    'config.process[0].model.qtype_te',
    'config.process[0].model.quantize_te',
  ],
] as const) {
  try {
    collectVisibleControlClaimsFromSource(
      quantizationFallbackSource(primaryPath, secondaryPath, "'garbage'"),
      'fixture.tsx',
      'Fixture',
    );
    factualUiContractFailures.push(label);
  } catch (error) {
    assert.match(String(error), /quantiz|qtype|string|accepted|fallback/i, `${label} must reject a non-option literal fallback`);
  }
}
for (const [label, fallback, imports] of [
  ['accepted qtype literal fallback remains valid', "'qfloat8'", ''],
  ['exact defaultQtype fallback remains valid', 'defaultQtype', "import { defaultQtype } from './options';"],
] as const) {
  try {
    const claims = collectVisibleControlClaimsFromSource(
      quantizationFallbackSource(
        'config.process[0].model.qtype',
        'config.process[0].model.quantize',
        fallback,
        imports,
    ...
    const qtype = claims.find(claim => claim.path === 'config.process[*].model.qtype');
    if (JSON.stringify(qtype?.value_contract.accepted_values) !== JSON.stringify([
      { kind: 'string', value: '' },
      { kind: 'string', value: 'qfloat8' },
    ])) factualUiContractFailures.push(label);
  } catch {
...
for (const [label, valueExpression] of [
  [
    'whole-expression division cannot masquerade as a 100x slider read scale',
    `(${layerScaleReadPath} * 100) / 100`,
  ],
  [
    'unknown whole-expression transform cannot masquerade as a 100x slider read scale',
    `unscale(${layerScaleReadPath} * 100)`,
  ],
] as const) {
  try {
    collectVisibleControlClaimsFromSource(
    ...
    assert.match(String(error), /reciprocal|scale|transform|expression/i, `${label} must prove the complete reachable value expression`);
  }
}
for (const [label, sourceText] of [
  [
    'parenthesized exact slider read scale remains provable',
    layerScaleSource.replace(layerScaleValueExpression, `((${layerScaleReadPath}) * (100))`),
  ],
  [
    'exact slider read alias remains provable',
    layerScaleSource.replace(
      'return <SliderInput',
      `const displayedFraction = ${layerScaleReadPath};\n    return <SliderInput`,
    ).replace(layerScaleValueExpression, 'displayedFraction * 100'),
  ],
] as const) {
  try {
...
for (const [label, corruptedCallback] of [
  [
    'invoked arrow helper overwrite invalidates quantization selection dataflow',
    `onChange={value => {
              let selected = value;
              const overwrite = () => { selected = ''; };
              overwrite();
              if (selected === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              setJobConfig(selected, 'config.process[0].model.qtype');
  ...
  [
    'invoked function helper overwrite invalidates quantization selection dataflow',
    `onChange={value => {
              let selected = value;
              function overwrite() { selected = ''; }
              overwrite();
              if (selected === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              setJobConfig(selected, 'config.process[0].model.qtype');
  ...
  [
    'invoked aliased helper overwrite invalidates quantization selection dataflow',
    `onChange={value => {
              let selected = value;
              const overwrite = () => { selected = ''; };
              const invokeOverwrite = overwrite;
              invokeOverwrite();
              if (selected === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              setJobConfig(selected, 'config.process[0].model.qtype');
  ...
  try {
    ...
    assert.match(String(error), /quantiz|selection|provenance|qtype|invocation/i, `${label} must include synchronous helper effects`);
  }
}
for (const [label, callback] of [
  [
    'uninvoked selection-overwrite helper remains harmless',
    `onChange={value => {
              let selected = value;
              const uninvoked = () => { selected = ''; };
              if (selected === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              setJobConfig(selected, 'config.process[0].model.qtype');
            }}`,
  ...
  [
    'safe synchronously invoked helper alias preserves selection dataflow',
    `onChange={value => {
              let selected = value;
              const preserve = () => { selected = value; };
              const invokePreserve = preserve;
              invokePreserve();
              if (selected === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              setJobConfig(selected, 'config.process[0].model.qtype');
...
try {
  collectVisibleControlClaimsFromSource(
    guardedQuantizationSource.replace(
      exactTransformerQuantizationCallback,
      `onChange={value => {
              let selected = value;
              const overwrite = () => { selected = ''; };
              const preserve = () => { selected = value; };
              const invoke = condition ? overwrite : preserve;
              invoke();
              if (selected === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              setJobConfig(selected, 'config.process[0].model.qtype');
  ...
  factualUiContractFailures.push('conditionally unresolved synchronous helper invocation fails closed');
} catch (error) {
  assert.match(String(error), /quantiz|selection|provenance|invocation|tainted/i, 'unresolved helper invocation must fail closed');
}
...
for (const [label, callback] of [
  [
    'direct constructor callback corrupts quantization carrier',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { run(); } }
              new Base(() => { value = false; });
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'explicit derived super callback forwarding corrupts quantization carrier',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { run(); } }
              class Derived extends Base { constructor(run) { super(run); } }
              new Derived(() => { value = false; });
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'reordered finite super arguments forward corrupting callback',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(ignored, run) { run(); } }
              class Derived extends Base { constructor(run) { super('safe', run); } }
              new Derived(() => { value = false; });
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'implicit derived constructor forwards corrupting callback',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { run(); } }
  ...
  [
    'constructor callback invalid quantize setter effect fails closed',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { run(); } }
              new Base(() => setJobConfig('garbage', 'config.process[0].model.quantize'));
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
] as const) {
  try {
    ...
    assert.match(String(error), /constructor|invocation|quantiz|qtype|selection|payload|provenance/i, `${label} must replay constructor callback effects`);
  }
}

for (const [label, sourceText, symbol, projected] of [
  [
    'ordinary options mutation through constructor callback fails closed',
    ordinaryClassEffectControl(
      `class Base { constructor(run) { run(); } }`,
      `new Base(() => modeOptions.push({ value: 'b', label: 'B' }));`,
    ),
    'Fixture',
    false,
  ],
  [
    'projected options mutation through constructor callback fails closed',
    projectedOptionsControl(
      `class Base { constructor(run) { run(); } }
       new Base(() => groupedModelOptions.push({ value: 'stale', label: 'Stale' }));`,
      '',
      '',
    ),
    'SimpleJob',
  ...
  try {
    ...
    assert.match(String(error), /constructor|invocation|select.*options|accepted values|projected|provenance/i, `${label} must replay shared constructor effects`);
  }
}
...
try {
  collectVisibleControlClaimsFromSource(ordinaryClassEffectControl(
    `class Base { constructor(run) { run(); } }`,
    'new Base(externalCallback);',
  ), 'fixture.tsx', 'Fixture');
  factualUiContractFailures.push('unresolved constructor callback fails closed');
} catch (error) {
  assert.match(String(error), /constructor|callback|invocation|provenance/i, 'unresolved constructor callbacks must fail closed');
}

for (const [label, callback] of [
  [
    'safe invoked constructor callback remains harmless',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { run(); } }
  ...
  [
    'uninvoked corrupt constructor argument remains harmless',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { void run; } }
              new Base(() => { value = false; });
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'constructor primitive arguments remain harmless',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(kind, count) { void kind; void count; } }
              new Base('safe', 1);
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'safe explicit constructor inheritance remains harmless',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { run(); } }
  ...
  [
    'safe recursive constructor callback terminates',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { run(); } }
...
for (const [label, callback] of [
  [
    'constructor callback call wrapper corrupts quantization carrier',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { run.call(undefined); } }
              new Base(() => { value = false; });
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'constructor callback apply wrapper corrupts quantization carrier',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { run.apply(undefined, []); } }
              new Base(() => { value = false; });
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'constructor callback bind wrapper corrupts quantization carrier',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(run) { run.bind(undefined)(); } }
              new Base(() => { value = false; });
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'unresolved constructor member callback fails closed',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(holder) { holder.run(); } }
              new Base(externalHolder);
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'derived super unresolved constructor member callback fails closed',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(holder) { holder.run(); } }
              class Derived extends Base { constructor(holder) { super(holder); } }
              new Derived(externalHolder);
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
] as const) {
  try {
    ...
    assert.match(String(error), /constructor|callback|invocation|qtype|selection|payload|provenance/i, `${label} must replay or fail closed`);
  }
}

for (const [label, sourceText, symbol, projected] of [
  [
    'ordinary unresolved constructor index callback fails closed',
    ordinaryClassEffectControl(
      `class Base { constructor(holder) { holder[0](); } }`,
      'new Base(externalHolder);',
    ),
    'Fixture',
    false,
  ...
  [
    'projected detached constructor member callback fails closed',
    projectedOptionsControl(
      `class Base { constructor(holder) { const invoke = holder.run; invoke(); } }`,
      'new Base(externalHolder);',
      '',
    ),
    'SimpleJob',
  ...
  try {
    ...
    assert.match(String(error), /constructor|callback|invocation|select.*options|accepted values|projected|provenance/i, `${label} must fail closed on unresolved constructor-parameter calls`);
  }
}

for (const [label, callback] of [
  [
    'finite constructor member callback remains harmless',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(holder) { holder.run(); } }
              new Base({ run: () => undefined });
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'unrelated object method with constructor parameter remains harmless',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              const helper = { run: () => undefined };
              class Base { constructor(holder) { helper.run(); void holder; } }
              new Base(externalHolder);
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
  [
    'uninvoked constructor member callback remains harmless',
    `onChange={value => {
              if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
              else setJobConfig(true, 'config.process[0].model.quantize');
              class Base { constructor(holder) { void holder.run; } }
              new Base({ run: () => { value = false; } });
              setJobConfig(value, 'config.process[0].model.qtype');
            }}`,
  ],
] as const) {
...
for (const [label, helperBody] of [
  [
    'direct invoked helper rewrites constructor target before quantization construction',
    `const rewrite = () => { Ctor = Bad; };
               rewrite();`,
  ],
  [
    'aliased invoked helper rewrites constructor target before quantization construction',
    `const rewrite = () => { Ctor = Bad; };
               const invoke = rewrite;
               invoke();`,
  ],
  [
    'nested invoked helper rewrites constructor target before quantization construction',
    `const outer = () => {
                 const inner = () => { Ctor = Bad; };
                 inner();
               };
               outer();`,
  ],
  [
    'parameterized invoked helper rewrites constructor target before quantization construction',
    `const rewrite = Next => { Ctor = Next; };
               rewrite(Bad);`,
  ],
  [
    'unresolved invoked helper rewrite of constructor target fails closed',
    `const rewrite = () => { Ctor = externalCtor; };
               rewrite();`,
  ],
] as const) {
  const callback = `onChange={value => {
            if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
            else setJobConfig(true, 'config.process[0].model.quantize');
            class Safe {}
            class Bad { constructor() { value = false; } }
  ...
  try {
    ...
    assert.match(String(error), /class|constructor|new|qtype|selection|payload|provenance|target/i, `${label} must use the target state at new`);
  }
}

for (const [label, sourceText, symbol, projected] of [
  [
    'ordinary options mutation through helper-rewritten constructor target fails closed',
    ordinaryClassEffectControl(
      `class Safe {}
       class Bad { constructor() { modeOptions.push({ value: 'b', label: 'B' }); } }
       class Base {
  ...
  [
    'projected options mutation through parameterized helper-rewritten constructor target fails closed',
    projectedOptionsControl(
      `class Safe {}
       class Bad { constructor() { groupedModelOptions.push({ value: 'stale', label: 'Stale' }); } }
       class Base {
  ...
  try {
    ...
    assert.match(String(error), /class|constructor|new|select.*options|accepted values|projected|provenance|target/i, `${label} must use the target state at new`);
  }
}

for (const [label, assignment] of [
  ['direct constructor target reassignment remains closed', 'Ctor = Bad;'],
  ['destructuring constructor target reassignment remains closed', '[Ctor] = [Bad];'],
] as const) {
  const callback = `onChange={value => {
            if (value === '') setJobConfig(false, 'config.process[0].model.quantize');
            else setJobConfig(true, 'config.process[0].model.quantize');
            class Safe {}
            class Bad { constructor() { value = false; } }
            class Base { constructor(Ctor) { ${assignment} new Ctor(); } }
            new Base(Safe);
            setJobConfig(value, 'config.process[0].model.qtype');
          }}`;
  try {
    ...
    assert.match(String(error), /class|constructor|new|qtype|selection|payload|provenance|target/i, `${label} must not reuse the original constructor argument`);
  }
}

for (const [label, helperBody] of [
  [
    'uninvoked helper constructor target rewrite remains harmless',
    `const rewrite = () => { Ctor = Bad; };
               void rewrite;`,
  ],
  [
    'helper rewrite to the same safe constructor remains harmless',
    `const rewrite = () => { Ctor = Safe; };
               rewrite();`,
  ],
  [
    'helper rewrite to another exact safe constructor remains harmless',
    `class OtherSafe {}
               const rewrite = () => { Ctor = OtherSafe; };
               rewrite();`,
  ],
  [
    'unrelated target rewrite and local construction remain harmless',
    `class OtherSafe {}
               let Other = Safe;
               const rewrite = () => { Other = OtherSafe; };
               rewrite();
               new Other();`,
  ],
] as const) {
...
if (liveRoot !== undefined) {
  ...
  assert.equal(declaredTypeScriptSources.length, 155, 'every concrete TypeScript source matched by the declared globs is scanned');
...
${architectureCommit}`,
  ...
  assert.deepEqual(
    ...
    [
      ...
      'ui/src/server/trainingPresetCatalogBuildValidation.ts',
      'ui/src/utils/api.ts',
      'ui/src/utils/callScript.ts',
    ],
